...
import { IReportEmbedConfiguration, IEmbedConfiguration, models, Page, Report, service, VisualDescriptor } from 'powerbi-client';
import { PowerBIReportEmbedComponent } from 'powerbi-client-angular';
import { IEmbedParams } from '../shared/models/embed-params.model';
import { IEmbedReport } from '../shared/models/embed-report.model';
...
//https://github.com/microsoft/powerbi-client-angular/blob/main/Angular/demo/src/constants.ts
...
export class PowerBiEmbeddedComponent implements OnInit {
  ...
  displayMessage = 'Click button to load report.';
  ...
  reportList: IEmbedReport[] | undefined;
  ...
  constructor(public http: HttpClient, private element: ElementRef<HTMLDivElement>, @Inject('BASE_URL') baseUrl: string) {
  ...
  ngOnInit() {
    try {
      ...
      this.http.get<IEmbedReport[]>(this._baseUrl + reportListUrl).subscribe(reportList => {
   ...
   */
  async embedReport(reportId: string): Promise<void> {
    let reportConfigResponse: IEmbedParams;

    // Get the embed config from the service and set the reportConfigResponse
    try {
      if(!reportId) {
        reportConfigResponse = await this.http.get<IEmbedParams>(this._baseUrl + reportUrl).toPromise();
      } else {
        reportConfigResponse = await this.http.get<IEmbedParams>(this._baseUrl + reportUrl + "/" + reportId).toPromise();
      }
    ...
    this.paginatedReportConfig = {
      ...this.paginatedReportConfig,
      id: reportConfigResponse.report.reportId,
      embedUrl: reportConfigResponse.report.embedUrl,
      accessToken: reportConfigResponse.token.token,
      settings: {
        layoutType: models.LayoutType.Custom,
        customLayout: {
          displayOption: models.DisplayOption.FitToPage
